...
ggplot(DE_comparing_contrasts, aes(lfc, lfc2)) +
  ...
  facet_wrap(~condition, labeller = labeller(condition = cond_labs)) +
...
```
```{r}
# combine pairwise spearman cors with data on time differences between stages
ss_dat_time <- SScolData%>%
  ...
  distinct()%>%
  left_join(., 
            data.frame(lab = x_ax_lab, Condition = c("A", "B", "C", "D", "E", "F", "G", "H", "I", "L")))

DE_spear_long_plotX <- left_join(
  DE_spear_long_plot,
  ss_dat_time%>%rename(time1 = Time,
                       lab1 = lab),
  by = c("term" = "Condition")
)%>%
  left_join(
  .,
  ss_dat_time%>%rename(time2 = Time,
                       lab2 = lab),
  by = c("cond" = "Condition")
)%>%
  mutate(time_diff = time2 - time1,
         lab_comb = paste0(lab1, ":", lab2))%>%
  mutate(lab_comb2 = case_when(
    grepl(func_grp, pattern = "function") ~ paste0("*",lab_comb,"*"),
    !grepl(func_grp, pattern = "function") ~ lab_comb)
    )%>%
...
```{r}
DE_spear_long_plotX%>%
  ggplot(., aes(x = lab_comb2, y = med_rho)) + 
  geom_pointrange(aes(ymin = lwr_rho, ymax = upr_rho), shape = 3) +
  geom_point(aes(x = lab_comb2, y = obs_spearman, color = func_grp),
             shape = 15, size = 2) +
...
In the above plot, pairs are ordered by the type of comparison, starting with consecutive stages. Alternatively, we can order the correlations by the amount of time (days) that passes between stage sampling.

Here is that plot. Pairs of life stages at the top, which were sometimes sampled within minutes or hours of each other, tend to have similar gene expression (positive correlations), whereas pairs at the bottom, which were sampled weeks or months apart, tend to have more different gene expression (negative correlations).

```{r}
DE_spear_long_plotX%>%
  arrange(time_diff)%>%
  mutate(lab_comb = fct_inorder(lab_comb))%>%
  ggplot(., aes(x = lab_comb, y = med_rho)) + 
  geom_pointrange(aes(ymin = lwr_rho, ymax = upr_rho), shape = 3) +
  geom_point(aes(x = lab_comb, y = obs_spearman, color = func_grp),
             shape = 15, size = 2) +
  geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
  coord_flip() + 
  scale_x_discrete(limits = rev) +
...
This can also be seen in a bivariate plot of the time between sampling vs the spearman cor coefficient. Stages sampled temporally close together have higher correlations than those sampled further apart. But some pairs of stages have relatively higher or lower similarity (positive or negative residuals) than expected, and they are labelled.

```{r}
DE_spear_long_plotX%>%
  mutate(res_time_v_cor = residuals(lm(obs_spearman~time_diff, data = DE_spear_long_plotX)))%>%
  ggplot(., aes(time_diff, obs_spearman)) +
  geom_smooth(method = lm, se = F) +
  geom_point(aes(color = func_grp)) +
  geom_label(data = .%>%
               slice_max(res_time_v_cor, n = 7),
             aes(label = lab_comb, color = func_grp), size = 2) +
  geom_label(data = .%>%
               slice_min(res_time_v_cor, n = 7),
             aes(label = lab_comb, color = func_grp), size = 2) +
  labs(x = "Time diff (days)", y = "Spearman cor coef")
```

Ordering pairs by time elapsed between them results in stages being mixed along the axis. That is, the ordering of the pairs no longer follows the chrnology of the life cycle. An alternative is to separate the pairs by life stage and to order them chronologically within life stages. Here is that plot:

```{r}
DE_spear_long_plotX%>%
  mutate(lab1 = fct_inorder(lab1),
         lab_comb = fct_inorder(lab_comb))%>%
  ggplot(., aes(x = lab_comb, y = med_rho)) + 
  geom_pointrange(aes(ymin = lwr_rho, ymax = upr_rho), shape = 3) +
  geom_point(aes(x = lab_comb, y = obs_spearman, color = func_grp),
             shape = 15, size = 2) +
