...
One possible way to improve clustering would be to use only differentially expressed genes. Let's import the genes that differed in expression between at least two treatments.

```{r}
gc_degs <- read.csv(file = "../data/DEGsConditionM.csv", sep = " ")
```

Here are the number of DEGs at different levels of significance. A 10-fold change in the p-value usually removes roughly 1000 genes from the list.
...
#   geom_line() +
#   # scale_x_log10() +
#   scale_y_log10(limits = c(0.00000001, 0.05))
```

Let's only take the genes that had p-values less than 0.0001.

```{r}
degs <- filter(gc_degs, padj < 0.0001)%>%row.names()
```

Here is an elbow plot, based on kmeans clustering, with those genes.

```{r}
# function to compute total within-cluster sum of square 
wss <- function(k) {
  kmeans(
    gc_expressed_trans[row.names(gc_expressed_trans) %in% degs,],
...
qplot(k.values, wss_values) +
  ...
  labs(x = "Number of clusters K", y = "Total within-clusters sum of squares") 
```
To make the breaks clearer, here is the log-transformed plot. There are at least 3 clusters.
...
gene_dist <- dist(
  ...
  gc_expressed_trans[
    row.names(gc_expressed_trans) %in% degs,
    ]
  )
...
Given the elbow plot, let's take the top 9 clusters.

```{r}
clusters <- cutree(gene_hclust, k = 9)
...
Some of the trends might be easier to assess if we plot the expression profiles for individual genes. Let's sample 100 random genes from each cluster and plot them. This uncovers a little more nuance within the clusters, such that we can see why some were clusters were split (e.g. more variance at some stages than others).
...
As we saw above, the ward clustering algorithm seemed to perform well. Thus, let's try that on the same subset of DEGs, taking the top 15 clusters.
...
The variance within clusters is smaller than with the complete linkage approach, so it seems to be performing well. Some trends, though, may be redundant.
