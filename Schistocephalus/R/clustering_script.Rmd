...
[Elsewhere](SS_DEmodels.Rmd) we explored which genes to remove from the analysis due to low levels of expression. As an arbitrary threshold, we removed genes that do not have a median expression level of at least 10 counts in at least one condition.
...
Out of these, this number had low expression levels under our criteria above (because they were removed before fitting the models).
...
Instead of plotting the expression profiles as a function of discrete developmental stages, we can plot them as a function of age, which tacitly acknowledges that stages close together in an age should be similar.

```{r}
gc_expressed_trans_clusters_age <- left_join(
  gc_expressed_trans_clusters%>%
    ungroup(),
  md%>%
    mutate(Time = gsub(",", ".", Time))%>%
    mutate(Time = as.numeric(Time))%>%
    group_by(Condition)%>%
    summarize(age = mean(Time))%>%
    ungroup(),
  by = c("grp" = "Condition")
  )
```

Here are the same clusters as above but as a function of age. The groups that are close together in time, like copepod-transmission and fish-infecting, are no longer distinguishable. Big changes in expression in short time intervals are only really seen in the transition to the in vitro bird.

```{r}
ggplot(gc_expressed_trans_clusters_age%>%
         ungroup()%>%
         filter(gene %in% gene100_perclust), 
       aes(x = age, y = z_score)) +
  # background for copepod stage
  annotate("rect",
           xmin=0.02, xmax=14.1, ymin = -Inf, ymax = Inf,
           fill = "gray") +
  # background for bird stages
  annotate("rect",
           xmin=67.04, xmax=71.12, ymin = -Inf, ymax = Inf,
           fill = "gray") +
  geom_hline(yintercept = 0, linetype = "dashed") +
  geom_line(aes(group = gene), alpha = 0.1) +
  geom_line(stat = "summary",
            ...
            aes(group = 1)) +
  # scale_x_continuous(
  #   breaks = unique(gc_expressed_trans_clusters_age$age),
  #   labels = x_ax_lab, 
  #   guide = guide_axis(angle = 90)) +
  scale_color_distiller(type = "div", palette = "RdBu") +
  facet_wrap(~ as.factor(cluster)) +
...
The temporally uneven sampling across the life cycle is clearer if we plot points instead of lines.
...
New patterns do not obviously emerge when the number of clusters is expanded, though within-cluster variance probably decreases.
...
```
Changing the x-axis to age still does not make the patterns clearer.
...
These *clust* tests were not included in the Repo, so the code is commented out.
