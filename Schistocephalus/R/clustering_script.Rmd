...
So, when we take a conservative p-value threshold, many of these genes will be removed. Here are the number of DEGs at different levels of significance. A 10-fold change in the p-value usually removes roughly 1000 genes from the list.
...
Thus, the model with all developmental stages is probably the best. Let's assess this by looking the number of genes considered differentially expressed by different models.
...
```{r}
# grid.arrange(sample_map, heatmap, ncol = 1, heights = c(1,5))
```
